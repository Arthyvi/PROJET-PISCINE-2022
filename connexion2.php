<?php

// Declaration des variables
$id= isset($_POST["id"]) ? $_POST["id"] : "";
$mdp= isset($_POST["mdp"]) ? $_POST["mdp"] : "";

// Detection problème
if($id == "")
{
    echo "Probleme avec la valeur de l'identifiant<br>";
    return;
}

if($mdp == "")
{
    echo "Probleme avec la valeur du mot de passe<br>";
    return;
}


// Connexion au serveur
$mysqli = new mysqli("localhost:3306","root","","projet");

// Check connection
if($mysqli -> connect_errno)
{
    echo "Failed to connect to MySQL" . $mysqli -> connect_errno;
    exit();
}
else
{
    // Verification si Identifiant bon
    $sql = "SELECT * FROM identifiant";
    $BufferIDpersonne = "";
    

    if($result = $mysqli->query($sql))
    {
        
        if($result->num_rows >0)
        {
            while($row = $result->fetch_row())
            {
                if($row[0] == $id)
                {
                    $BufferIDpersonne = $row[1];
                    break;
                }
            }
        }

        $result->free_result();
    }

    if( $BufferIDpersonne == "")
    {
        echo "Identifiant ou mot de passe incorect (1)<br>";
        return;
    }

    //Definition si l'IDpersonne trouvé est : un adminisrateur (AD), un client (CL) ou un medecin (MD)

    $NameTable ="";

    switch(substr($BufferIDpersonne,0,2))
    {
        case "AD": // Si c'est un administrateur...
            $NameTable ="administrateur";
            break;

        case "CL" : // Si c'est un client...
            $NameTable ="client";
            break;

        case "MD" : // Si c'est un Medecin...
            $NameTable ="medecin";
            break;
    }


    //// Verification si Mot de passe bon

    $sql = "SELECT oth.Password1 FROM $NameTable oth WHERE IDpersonne='$BufferIDpersonne'";
    $Passed = false;

    if($result = $mysqli->query($sql))
    {
        
        if($result->num_rows >0)
        {
            while($row = $result->fetch_row())
            {
                if($row[0] == $mdp)
                {
                    $Passed = true;
                    break;
                }
            }
        }

        $result->free_result();
    }

    if( $Passed == false)
    {
        echo "Identifiant ou mot de passe incorect (2)<br>";
        return;
    }
    else
    {
        echo"<div class='page-section'>";
        echo"<div class='container'>";
        echo"<h1 class='text-center'>Vous êtes connecté</h1>";
        echo"</div>";
        echo"</div>";
        header('Location: home.php');
        
    }

    // Fermeture de notre variable "$mysqli"
    $mysqli->close();
    
}
?>
